...
<!--headr-->
 ...
 <img src="src/img/banner1.jpg" alt="sang "  class="image-banner" >  
    ...
    <h1 class="title"> Liste DE Recherche De Demande Sang</h1>
    <br>
    <br>
     <!---banner-->
...
$resulta=mysqli_query($conn,"SELECT * FROM visiteur  WHERE Groupe_de_sang='".$groupesang."' and 	adresse='".$adresse."'  and	Type=2");
$nr=mysqli_num_rows($resulta);

if($nr==0){ 
  echo "<script>alert('le groupe de sang qui va recherche il existe  pas '); </script>";
  header("location: recherche.php");
}
else{
  echo " <table   style=' width:95% ; text-align:center; border:1px;margin-left:22px;height:80px;'>\n" ;
 echo"<tr style=' background-color:burlywood; color:brown;height:40px;' >\n";
    echo"<th class='titletabl'>Numéro de Donneur</th>\n";
             ...
             echo"  <th class='titletabl'> établissement</th>\n";
...
while($row = $resulta->fetch_assoc()){
  ...
  echo"<th style=' background-color:brown;color:white;' class='titletabl'>$row[id_visiteur]</th>\n";
           ...
           echo"  <th style=' background-color:brown;color:white;' class='titletabl'>$row[établissement]</th>\n";
