...
<!--headr-->
 ...
 <img src="src/img/banner1.jpg" alt="sang "  class="image-banner" >  
    ...
    <h1 class="title">Ajouter un don de sang </h1>
     <!---banner-->
    ...
    <form class="modal-content3 " method="POST">
         ...
         <strong>Sexe</strong><br>
          ...
          <strong>Groupe Sanguin : </strong><br>
           ...
           <br>
           <strong> Date de dernier don:</strong><br>
            <input type="date" placeholder="Entrer votre date de donné" name="day" required>
            <br>
...
<option> <?php   echo $row["wilaya"];?> </option>
       <?php } ?>
           ...
           <div class="container4" > 
               <button type="submit" value="submit" class="btnn btnvalider1">Valider</button>
...
if(isset($_POST['name']) || isset($_POST['prenom']) || isset($_POST['age'])  || isset($_POST['sexe'])  || isset($_POST['groupesang'])  || isset($_POST['day'])  || isset($_POST['number'])|| isset($_POST['email'])  || isset($_POST['adresse'])  ){
$name=$_POST['name'];
$prenom=$_POST['prenom'];
$age=$_POST['age'];
$sexe=$_POST['sexe'];
...
if((!empty($name) )|| (!empty($prenom))||(!empty($age)) ||(!empty($sexe)) ||(!empty($groupesang))||(!empty($day))|| (!empty($number)) ||(!empty($email))||(!empty($adresse)))
{
  $dbhost="localhost";
  $dbuser="root";
  $dbpass="";
...
$insert="INSERT INTO visiteur(Nom ,Prenom,Age,Sexe,Groupe_de_sang,Date_de_donne,	Numero,email,adresse,Type) value (?,?,?,?,?,?,?,?,?,1)";


$stmt=$conn->prepare($insert);
  $stmt->bind_param("ssisssiss",$name,$prenom,$age,$sexe,$groupesang,$day,$number,$email,$adresse);
  $stmt->execute();
